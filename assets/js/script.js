...
function getCurrentDay(userInput) {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${userInput}&appid=1df7696f823ad8cc7efbb5f9a31ff2b8&units=imperial`)
...
function getFiveDay(userInput) {
    fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${userInput}&appid=1df7696f823ad8cc7efbb5f9a31ff2b8&units=imperial`)
        .then(function (response) { //server response
            return response.json();
        })
        .then(function (data) {
...
function displayFiveDay(fiveDayObject, filteredArray) {
    ...
    for (let i = 0; i < filteredArray.length; i++) { //loops through the 5 days in the array and grab what we need
        ...
        let dateHeader = document.createElement('h4') //create h element to append city name
        ...
        const weatherCardP = weatherCardLines.map(line => document.createElement('h5')); //create a new h5 element for each string in the array, map through each string

        weatherCardP.forEach((h5, index) => {
            h5.textContent = weatherCardLines[index];
        }); //for each string in the new array, set the h5 to display the text
